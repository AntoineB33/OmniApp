...
### The resulting share, and what test 12 gives task A

**The resulting share is one number**: what the drawn timeline gave a task, as a fraction of the time **some** task was allowed to run in (`scheduler_logic.resulting_shares`). An instant every period refuses is not time a schedule failed to use — it is time the schedule was never offered — so the nights are cut out of the numerator and the denominator alike, and what is missing from the sum of all the tasks is time that *was* offered and left empty. Both the window and `--verify` report that same measure, so the two cannot disagree; the earlier pair of numbers ("of the cycle", "of the drawn timeline") could and did — on a case whose links have no cycle at all, the first fell back to the **prefix**, quoting a one-off catch-up burst measured over an environment truncated at the 6-hour lookahead: 33.79% for a task the timeline underneath it was giving 2%.

**The clocks are replayed over schedulable time, not wall time** (`Scheduler._lookback_start`). The window the past is read off is measured in the only currency the shares are about. Measured in wall time, an instant nobody may run in pushes real service out of the window — and then a task served just before the last long exclusion reads as *never served* and leapfrogs the one that has been waiting. Over a timeline whose nights take nine hours out of every twenty-four that is not a rounding error: each of the twenty low-priority tasks came back due once per wall-clock window, spent most of the usable time on its 45-minute minimum, and A collected what was left — **91 minutes over three days, 2% against a 50% target.** With the window measured properly, A gets **993 minutes, 34.7%**, which is what one uninterrupted plan over the same environment gives (35.4%) — the chain no longer forgets what the plan remembers.

**An exclusion no longer erases the ranking of everyone it excludes** (`Scheduler._relax`). An excluded task's clock is still held within one period of the served pool — an imbalance older than that is forgotten, which is what stops a long exclusion buying an equally long catch-up — but the whole excluded set is now **translated** rather than each of its members clamped separately. Clamping set every task past the bound to the *same* value, so a privileged-only period refusing eleven tasks left them tied and their priorities stopped deciding which went first; measured, eleven distinct clocks collapsed to one at every such period.

**A cannot reach 50% here, and the arithmetic says so.** Inside a period of `max(mᵢ/pᵢ)` = 30 hours the twenty other tasks must each be given a 45-minute minimum first — 900 minutes — so A's 50% needs 1800 minutes of *schedulable* time per cycle, and three days of this environment hold about 2450 of it in total. A's ceiling is therefore ~50% of the schedulable timeline and its floor is set by how often a 45-minute slot of it fits at all.

**Still open: the standing grid ahead of the line.** At $t_p$ = 24 h the far part of the timeline is drawn from the environment *standing* (the whole break grid, approximation 1 above), and there A still gets 3% of the schedulable time while the privileged tasks take 67%. The cause is not the clocks — A's is the lowest — but that A never **fits**: with a privileged-only stretch every hour, a 45-minute slot of A only fits in the few minutes after one ends, and those are the minutes the run that started at the end of it is still holding. Declining to *start* a run that can only end by eating a hungrier rival's one chance (idling until the rival is placeable) was measured to fix it — A 3% → 16%, the privileged 67% → 32% — at 23% of the timeline left idle. It is not shipped, because its trigger is a **threshold** in $t_p$ and that makes the affine-rule fitting fail (test 11 at $t_p$ = 38 min): `SlidingRules._fit` never probes a regime at the position it is claimed *from*, and the bisection that found that position only brackets the true breakpoint, so every regime owns a sliver on the wrong side of its own start. Probing `lo` exposes the hole but cannot close it — the range that then fails is narrower than `MIN_RANGE`, and `MovingWindow` has no representation for a single-**point** regime the way `ProgressiveWindow` does. Giving it one is the prerequisite; the note sits on `PROBES`.