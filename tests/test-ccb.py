...
class Task:
    def __init__(self, name: str, priority: float, min_time: float, needs_screen: bool):
        ...
        # FFQ (Fluid Fair Queuing) State Tracking
        self.r = 0.0          # Target proportion of active time (0.0 to 1.0)
        self.executed = 0.0   # Total minutes this task has run
...
def schedule_timeline_raw(tasks: list[Task], timeline: Iterable[TimeBlock]) -> Iterator[dict]:
    ...
    for tk in tasks:
        tk.r = tk.priority / total_priority
        tk.executed = 0.0
        
    total_active_time = 0.0
    t = 0.0
    
    def get_credit(task: Task) -> float:
        # Credit = Ideal Time - Actual Time
        return (total_active_time * task.r) - task.executed
    ...
    while True:
        ...
        # 1. Filter Eligible Tasks (Must be valid AND have enough continuous space)
        eligible = []
        for tk in tasks:
            if is_task_valid_for_period(tk, current_block["type"]):
                ...
                if allowed_space >= tk.min_time:
                    eligible.append((tk, allowed_space))
        ...
        # 3. Select Highest Credit (with Boundary Deferral Heuristic)
        time_to_boundary = stream.get_time_until_next_boundary(t)
        
        # Sort all eligible tasks by credit
        eligible.sort(key=lambda x: get_credit(x[0]), reverse=True)
        
        # Find tasks that can run without crossing into the next block
        fitting_tasks = [x for x in eligible if x[0].min_time <= time_to_boundary]
        
        if fitting_tasks and eligible[0][0].min_time > time_to_boundary:
            # The highest credit task would cross the boundary, but a shorter task fits.
            # Defer the crossing task to prevent fragmentation.
            winner, max_allowed = fitting_tasks[0]
        else:
            # Otherwise, just pick the highest credit task
            winner, max_allowed = eligible[0]
        
        # 4. Calculate Ideal Scheduling Duration
        dt_eq = float('inf')
        c_winner = get_credit(winner)
        for other, _ in eligible:
            if other == winner: continue
            c_other = get_credit(other)
            den = 1.0 - winner.r + other.r
            if den > 0:
                dt = (c_winner - c_other) / den
                if 0 <= dt < dt_eq:
                    dt_eq = dt
                    
        boundary_limit = max(winner.min_time, time_to_boundary)
        
        # Constrain duration logically
        duration = max(winner.min_time, min(dt_eq, max_allowed, boundary_limit))
        
        # Anti-fragmentation heuristic: 
        # If the remaining continuous time space is too small for ANY task, 
        # extend this task to fill the gap so we don't cause idle dead time.
        if max_allowed != float('inf'):
            remaining = max_allowed - duration
            if 0 < remaining < min((tk.min_time for tk in tasks), default=0):
                duration = max_allowed

        # Prevent infinite chunks in fully infinite timelines for yielding purposes
        duration = min(duration, 1440.0) 
        duration = math.ceil(duration)
        
        # Update metrics
        winner.executed += duration
        total_active_time += duration
        
        # Yield the chunk. 
        # If it crosses a boundary, yield it split by boundaries for visualizer accuracy, 
        # but logically it was scheduled as one continuous block.
        rem = duration
        while rem > 0:
...
if __name__ == "__main__":
    ...
    # The new FFQ algo easily crosses boundaries and perfectly minimizes discrepancy.
    tasks_1 = [
    ...
    print_colored_timeline("FFQ Algo - Perfect 1h/1h Balancing", schedule_1_slice)
    ...
    print_colored_timeline("FFQ Algo - Custom Daily Pattern", schedule_2_slice)
