...
REM  seconds after the rest that discharges it - that interval IS its
REM  recurrence bar. Both knobs are forwarded to BOTH surfaces so they agree -
REM  the desktop gradle launch honors -Pomniapp.breakDurationMs /
REM  -Pomniapp.breakIntervalMs (account1-empty-and-open.bat forwards
REM  OMNIAPP_BREAK_DURATION_MS / OMNIAPP_BREAK_INTERVAL_MS when set), and the
REM  phone honors the matching omniapp_break_* launch extras
REM  (internal\deploy-android-debug.bat).
