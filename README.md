...
- **`update-supabase-cli.bat`** — updates the Supabase CLI **tool** (never the database), whichever way it was installed: it detects what `PATH` actually resolves `supabase` to and either runs `npm install -g supabase@latest` or downloads the latest standalone release. If two installs are on `PATH` it says which one wins and fails if it updated the shadowed one — see CLAUDE.md.
