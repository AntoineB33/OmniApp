...
- **`update-supabase-cli.bat`** — updates a *standalone* Supabase CLI binary in place. Only useful if that is how you installed the CLI; if `where supabase` resolves to an npm global (`%APPDATA%\npm\supabase.cmd`), update with `npm install -g supabase@latest` instead — see CLAUDE.md.
