...
/**
 * Web persistence (PRD §5): SQLite is not wired on wasmJs yet, so the whole [PersistedSnapshot] (state
 * payload + per-unit history rows) is serialized to a single browser-localStorage entry.
 */
private class WasmLocalStorageSchedulerStore : SchedulerStore {
...
actual fun createDefaultSchedulerStore(): SchedulerStore? = WasmLocalStorageSchedulerStore()
