...
/**
 * PRD §15 / ARCHITECTURE.md §8 requirements #2/#6 (iOS APNs): hand the platform the two callbacks its native
 * push layer needs — [registerApnsToken] to publish this phone's APNs token (`device_push_token`), and
 * [onRemotePush] to act on a received `{action, due_at}` remote push. `App()` calls this once after building
 * the engine; only the iOS actual retains them (for a Swift AppDelegate to invoke via `IosPushBridge`), every
 * other platform is a no-op (Android routes the same callbacks through its FirebaseMessagingService instead).
 */
