...
/**
 * PRD §14 Chores Manager entry: one row of the standalone chores list — a [title], its recurrence
 * [spanDays] (the "spanning time", in days; a floating-point number > 1), and [timeOfDayMinutes] (the
 * "time in the day" the chore is placed, as minutes since local midnight in `0..1439`). Independent of
 * the task tree: the chores list is its own top-level state, not a per-task field. The chore scheduler
 * turns each entry into recurring 5-minute calendar panels (see
 * [org.example.project.scheduler.domain.SchedulerDomain.choreScheduledPanels]).
 */
...
data class TaskPanel(
    ...
    /**
     * PRD §14 Chores Manager: true for a 5-minute panel generated by the chore scheduler (its [taskId]
     * is null). Toward the §9 task scheduler a chore panel behaves like a [pinned] one — the auto fill
     * keeps it and flows around it. Toward the chore scheduler it has its own pin system: a chore panel
     * the user pinned ([pinned] = true) is kept when the chore list is regenerated, the rest are replaced.
     */
