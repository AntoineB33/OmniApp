...
class SchedulerStoreTest {
    ...
    /**
     * PRD §8/§15: an "I'm away" EPISODE is a recorded fact and must round-trip. The row is keyed by its
     * START, so opening the episode, every beat that extends it and its close are all the same row — an
     * episode can never come back as a pile of fragments.
     */
    @Test
    fun declared_away_spans_round_trip_and_extend_one_row() {
        val store = newStore()
        assertTrue(store.loadDeclaredAwaySpans().isEmpty())

        // The press opens it degenerate, the beat extends it, the "I'm back" closes it: one row throughout.
        store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(1_000, 1_000))
        store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(1_000, 31_000))
        store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(1_000, 44_000))
        assertEquals(listOf(DeclaredAwaySpanRecord(1_000, 44_000)), store.loadDeclaredAwaySpans())

        store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(90_000, 120_000))
        assertEquals(
            listOf(DeclaredAwaySpanRecord(1_000, 44_000), DeclaredAwaySpanRecord(90_000, 120_000)),
            store.loadDeclaredAwaySpans(),
            "episodes come back oldest first",
        )

        // Pruning drops what had already ENDED at the floor and keeps what straddles it.
        store.pruneDeclaredAwaySpans(50_000)
        assertEquals(listOf(DeclaredAwaySpanRecord(90_000, 120_000)), store.loadDeclaredAwaySpans())
    }

    /**
     * The away episodes are LOCAL-ONLY, like the sleep-scan checkpoint: writing one must not change the
     * [org.example.project.scheduler.persistence.PersistedSnapshot] the store loads (and syncs).
     */
    @Test
    fun declared_away_spans_do_not_affect_persisted_snapshot() {
        val store = newStore()
        store.save(SchedulerStateCodec.encodeSnapshot(stateWithHistory()))
        val before = store.load()!!

        store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(1_000, 2_000))

        assertEquals(before, store.load()!!)
    }

    /**
     * Persisted-DB compatibility (CLAUDE.md): a DB written by the *previous* schema (v12 — everything
     * through `history_unit.window`, no `device_away_span`) must still load, with 12.sqm adding the table
     * on open and the existing data untouched.
     */
    @Test
    fun upgrades_pre_away_span_v12_db_and_preserves_data() {
        val dbFile = File.createTempFile("scheduler-v12", ".db").also { it.delete() }
        try {
            val payload = SchedulerStateCodec.encodeSnapshot(stateWithHistory()).statePayload
            val url = "jdbc:sqlite:${dbFile.absolutePath}"
            val raw = JdbcSqliteDriver(url, Properties())
            // The v12 shape, exactly as 11.sqm left it.
            raw.execute(null, "CREATE TABLE app_state (account_id TEXT NOT NULL PRIMARY KEY, payload TEXT NOT NULL)", 0)
            raw.execute(
                null,
                "CREATE TABLE history_unit (account_id TEXT NOT NULL, category TEXT NOT NULL, " +
            ...
            raw.execute(null, "PRAGMA user_version = 12", 0)
            raw.close()
            ...
            // An upgraded DB has no away episodes on record, and the new table is usable at once.
            assertTrue(store.loadDeclaredAwaySpans().isEmpty())
            store.saveDeclaredAwaySpan(DeclaredAwaySpanRecord(5_000, 6_000))
            assertEquals(listOf(DeclaredAwaySpanRecord(5_000, 6_000)), store.loadDeclaredAwaySpans())
            driver.close()
        } finally {
            dbFile.delete()
        }
