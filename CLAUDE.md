...
| **Local-only view state** | focused window, tree selection, the "All tasks" window's own expansion/selection/edit session, `showScreenBreaks`/`showReminders`, WindowNav/Selection history, window placement (its filled axes and reduced state included) and the Search window's query and checked kinds, OS-sleep scan checkpoint, the device's work-offline choice, the refused-edit notice (`categoryRuleError`, not even persisted) | persist locally, **never sync** |
