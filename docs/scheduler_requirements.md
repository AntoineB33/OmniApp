...
* The influence of debt repayment decays over the distance from the blockage (both backward and forward in time). Because the compensation scales exponentially rather than linearly, massive blockages saturate the compensation limits rather than scaling proportionally (e.g., a 48-hour pre-placed task will increase a deprived task's presence in the adjacent schedule, but not twice as much as a 24-hour blockage). The goal is to avoid repaying debts indefinitely in favor of stabilizing the schedule.
