...
* **Add a checked reminder:** Right-clicking anywhere on the calendar offers, alongside "add a task", the option **"add a checked reminder"**. It opens a floating window with a **title input field** that has a **title-suggestion menu** and a **reminder id menu** (listing existing reminders — picking one fills the title — mirroring the task add window, because reminders have a title and an id), and a **time input field on its right** pre-filled with the time of day at the right-clicked position. **Cancel** and **Save** buttons sit at the bottom-right. Saving places an already-**checked** reminder tag for the chosen id at that time (recorded as a History Unit), which naturally **updates the schedule of the reminders of the same id**: that completion is kept across reminder regeneration and feeds the reminder schedule (reminders are scheduled like tasks — satisfying the target dispersion at the smallest level possible, with past reminders acting as tie-breakers).
