...
* **Undo / Redo:**
  * **Undo:** The history pointer decrements. The Delta is applied to the state, and the Delta *inside the unit* is mathematically inverted to represent the forward-change (Redo). For selection history, undo is `Alt + Left` and redo is `Alt + Right`. For the other history categories, undo is `Ctrl + Z` and redo is `Ctrl + Y`. The pointer navigates between history units according to the context: if the user is in Edit Mode, the pointer skips all history units that are not about the changes of the text content in this specific Edit Mode session. The same happens for the calendar: if the calendar is in focus, the pointer skips all other history units, and it skips all history units about the calendar if the calendar is not in focus.
...
* **Manual add:** Right-click anywhere on the calendar opens a contextual menu with the option "add a task" that adds a task at the right-click position in the calendar. The task is the one with the biggest absolute priority percentage, and first in the alphabetic order if there is a tie. The spanning time is the minimum time of the task.
* **edit window:** Double-click on a task in the calendar opens a floating window over the calendar floating window and the task tree. This window has an input field that is similar to the cells in `Change Task` mode in the tree task: same Edit Mode with the task menu and title suggestion menu. Like in the task tree, "New task" is selected by default. Creating a new task in the calendar doesn't create one in the task tree. There are also two input fields for the beginning time and the ending time.
* **Manual drags:** The user can drag a task in the calendar by double-click it and dragging while still holding the click. When dragging the task over other task periods, there must not be overlaps, so as the mouse come over consecutive tasks, the dragged task stay stick to the end of this group, and as soon as the mouse gets closer to the sart of the group than its end, the dragged task appears before the group. When it appears in gaps that are shorter than the dragged task, the dragged task shorten to fit, but remembers its original side for when it appears in wider gaps. The move is saved in a history unit only when the user releases the click.
