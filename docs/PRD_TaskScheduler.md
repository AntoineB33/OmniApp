...
## 14. Reminders
* **Window:** The reminders manager is a floating window holding a vertical list of rows; each row is three input fields — a title, a recurrence and a time of day. The recurrence has **days** as a unit and accepts a **formula** (an arithmetic expression — `+ - * /`, parentheses, decimals — e.g. `31/21`) as well as a plain number; the field keeps the raw formula so it round-trips, while its evaluated floating-point result is the cadence. The result is read as: **≥ 1** is a day cadence (`2` = alternate days; `31/21` ≈ 21 days out of every 31); **between 0 and 1** is **every day**; **≤ 0 or blank** is a **one-off** placed today only (so a title alone makes a single reminder).
