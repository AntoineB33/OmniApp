...
## 14. Reminders
* **Window:** The reminders manager is a floating window holding a vertical list of rows; each row is a title, a recurrence and a time of day. The recurrence is a number input with a **unit dropdown** at its right: **days** (default), **months** (every `n × 365.24219 / 12` days) or **years** (every `n × 365.24219` days) — months/years use the mean Gregorian year length. The number accepts a **formula** (an arithmetic expression — `+ - * /`, parentheses, decimals — e.g. `31/21`) as well as a plain number; the field keeps the raw formula and the chosen unit so they round-trip, while the cadence in days is the evaluated number times the unit. When the field holds a formula, its evaluated result is shown just to the right of the field, rounded to two decimals with a comma separator (e.g. `30/21` → `=1,43`). The result is read as: **≥ 1** is a day cadence (`2` = alternate days; `31/21` ≈ 21 days out of every 31); **between 0 and 1** is **every day**; **≤ 0 or blank** is a **one-off** placed today only (so a title alone makes a single reminder).
