...
## 14. Reminders
* **Window:** The reminders manager is a floating window holding a vertical list of rows; each row is three input fields — a title, a recurrence and a time of day. The recurrence has days as a unit (a floating point number superior to 1).
* **Calendar tags:** A reminder is *not* a task panel and has no spanning time: it shows on the calendar as a checkable tag (a zero-duration marker) placed at its time of day, recurring every n days. The calendar updates each time the reminders manager window changes. Recurrence is computed as before: if n is a floating point number, it is accumulated at each iteration and for every iteration the closest integer indicates the chosen day; if there were no iterations before or too far in the past, the accumulated counter starts from today. Because a reminder has no spanning time it is **not** an obstacle to the task scheduler (§9) — unlike the panels it replaces it never blocks, reshapes, or is avoided by the auto schedule. Reminder tags scheduled at the same time share their width evenly.
...
## 15. Side tasks
* **Definition:** Side tasks are for **eye health care**. They are placed on the calendar with a **real spanning time** (unlike reminders §14, which have none), materialized like a task panel showing their title. There are two kinds: the cadence micro-break (the look-away) and the rest pauses.
* **Cadence micro-break (look-away):** Recurs every 20 minutes, counted **from the last time the computer woke after a sleep longer than 15 minutes** (the OS sleep/wake history — on Windows the Kernel-Power sleep events 42/506 and wake events 1/131/507). So its clock restarts when the user gets back to the machine; if no such wake is known it falls back to the local-midnight grid.
* **Rest pauses (driven by the sleep history):** The 5-minute and 15-minute pauses are *not* a fixed grid — they are due based on whether the user has actually rested, which the device sleep history shows (a device sleep of at least the pause length counts as taking that pause). **If it has been more than an hour since the user last took a 5-minute pause, the 5-minute pause is scheduled now; likewise the 15-minute pause is scheduled now when it has been more than two hours since the last 15-minute pause.** While the user is resting enough, the pauses don't appear. When the user takes the pause (the device sleeps long enough), it is satisfied and stops being scheduled.
...
* **The side tasks:**
  - "look 20 feet away" — every 20 min, for 20 seconds (cadence micro-break).
  - "take a 5min pose and blink hard" — every 1 h, for 5 min.
  - "take a 15min pose" — every 2 h, for 15 min.