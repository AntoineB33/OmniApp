...
* **Text document ("see text"):** "see text" opens a floating window holding a free-form, multi-line text document associated with the task id (shared across every cell pointing at that task). The text is empty until the user writes something. The editor **auto-saves**: there is no Save button — every text change is persisted immediately, and **each change becomes its own History Unit** (undoable, §6), exactly like editing a task-tree cell's text in Edit Mode. The window is dismissed by clicking outside it; typing inside it (including the space bar) never closes it.
