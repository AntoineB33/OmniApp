...
## 14. Reminders
* **Window:** The reminders manager is a floating window holding a vertical list of rows; each row is a title, a recurrence and a time of day. The recurrence is a number input with a **unit dropdown** at its right. Interval units — **days** (default), **months** (every `n × 365.24219 / 12` days) or **years** (every `n × 365.24219` days) — and rate units — **per month** (`n` times a month → every `365.24219 / 12 ÷ n` days) or **per year** (`n` times a year → every `365.24219 ÷ n` days). Months/years use the mean Gregorian year length. The number accepts a **formula** (an arithmetic expression — `+ - * /`, parentheses, decimals — e.g. `31/21`) as well as a plain number; the field keeps the raw formula and the chosen unit so they round-trip, while the cadence in days is the evaluated number times the unit. When the field holds a formula, its evaluated result is shown just to the right of the field, rounded to two decimals with a comma separator (e.g. `30/21` → `=1,43`). The result is read as: **≥ 1** is a day cadence (`2` = alternate days; `31/21` ≈ 21 days out of every 31); **between 0 and 1** is **every day**; **≤ 0 or blank** is a **one-off** placed today only (so a title alone makes a single reminder).
* **Calendar tags:** A reminder is *not* a task panel and has no spanning time: it shows on the calendar as a checkable tag (a zero-duration marker) placed at its time of day — or at the **current time** when the time-of-day field is left blank — recurring per the rule above. The calendar updates each time the reminders manager window changes. For a cadence ≥ 1 the value is accumulated at each iteration and for every iteration the closest integer indicates the chosen day (so a fractional cadence hits its exact count over the smallest whole-number window without drifting); if there were no iterations before or too far in the past, the accumulated counter starts from today. Because a reminder has no spanning time it is **not** an obstacle to the task scheduler (§9) — unlike the panels it replaces it never blocks, reshapes, or is avoided by the auto schedule. Reminder tags scheduled at the same time share their width evenly.
...
* **Absorption:** when a shorter pause's window falls inside a longer pause that is already happening, the shorter one is **covered** by the longer (taking the longer pose counts as taking it too), so it is not drawn separately — its own rhythm continues unbroken.
...
* **Example** (the user has taken no pause for 3 hours, so at `t0` everything is overdue; look-away times rounded — the 20 s length is omitted): one **15-minute** pause starting at `t0`, a look-away at `t0+35min`, a look-away at `t0+55min`, a **5-minute** pose at `t0+1h15`, a look-away at `t0+1h35`, one at `t0+1h55`, a **15-minute** pose at `t0+2h15`, a look-away at `t0+2h35`, and so on. (The look-aways that would fall at `t0+1h15` and `t0+2h15` are absorbed by the 5- and 15-minute poses; the 5-minute pose that would fall near `t0+2h20` is absorbed by the 15-minute pose at `t0+2h15`.)
...
* **Notifications stay on:** side tasks are **notified by a system notification when they come due** (named by their title — the look-away / pose), and this is driven from the schedule, not the display. **Hiding the side tasks never silences their notifications.**
* **The side tasks:**
