...
* **Zoom:** The user can zoom the calendar vertically (the hour-row height scales) with the standard commands while the calendar is in focus: `Ctrl` + `+`/`-` zooms in/out (numpad `+`/`-` too), `Ctrl` + `0` resets to the default zoom, and `Ctrl` + mouse scroll zooms (scroll up to zoom in, down to zoom out) instead of scrolling the grid. The zoom is **directed at the mouse**: the point in time under the cursor stays under the cursor as the grid scales (for keyboard zoom with the cursor outside the grid, it pivots around the viewport centre). Zoom is clamped between a half and four times the default.
...
## 15. Side tasks
* **Definition:** Side tasks are tasks to do periodically, placed on the calendar with a **real spanning time** (unlike reminders §14, which have none). Each side task recurs at a fixed interval for a fixed duration, materialized on the calendar like a task panel showing its title.
...
* **The side tasks:**
  - "look 20 feet away" — every 20 min, for 20 seconds.
